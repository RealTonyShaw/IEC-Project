...
public class NoloVR_WinPlayform : NoloVR_Playform
{
    ...
    public override bool InitDevice()
    {
        ...
        try
        {
            ...
            //string path = Application.dataPath + "/server/NoloServer.exe";
#if UNITY_EDITOR
            string path = Application.dataPath + "/Plugins/x86_64/NoloServer.exe";
#else
             string path = Application.dataPath + "/Plugins/NoloServer.exe";
#endif
            bool result = NoloClientLib.StartNoloServer(path);
            if (result)
            {
            ...
            {
                Debug.LogError("StartNoloServer with error："+ path);
            }
